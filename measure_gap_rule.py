...
def main(argv=None):
    """Parsed FIRST, so `--help` costs nothing.

    This did its work at module level, so `--help` loaded the corpus and
    enumerated every candidate pair. self_check runs every offline script
    with --help assuming it is free, and caught this at 120 seconds.

    Same defect fixed in watchdog_shutdown.py earlier the same day, and
    reintroduced by the person who fixed it. A convention known and even
    enforced is still not automatic. The check is what holds it.
    """
    ...
    print(f"\n{over25 - sum(rescued.values()):,} candidate pairs MATCH his rule.")
    print("That is NOT the number whose treatment changes. Pairs already")
    print("beyond the 60-year window are refused today regardless, so the")
    print("increment is smaller: measured at 3,616,328, 25.0% of candidates,")
    print("by diffing blocked-context across a real A/B rather than counting")
    print("matches here. HANDOVER sec.9 rule 4: a block count is not an impact.")
    return 0
