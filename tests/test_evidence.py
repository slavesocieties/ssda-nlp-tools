...
# --------------------------------------------------------------------------
# Conflicting relationships (Daniel, 2026-08-07)
#
...
#
# Before this, a DIFFERENT spouse scored +0.00 -- byte-identical to knowing
# nothing. These pin the ruling, and pin the false positive that auditing the
# first A/B caught.
# --------------------------------------------------------------------------
...
def test_different_spouse_is_not_the_same_as_no_information():
    s = _stats(["maria"] * 40 + ["juan perez", "pedro gomez"])
    silent = network_llr(_m("maria", 1850), _m("maria", 1852), s)[0]
    clash = network_llr(_m("maria", 1850, spouse="juan perez"),
                        _m("maria", 1852, spouse="pedro gomez"), s)[0]
    assert silent == 0.0
    assert clash < silent, "a different spouse must cost something"


def test_spouse_conflict_decays_once_remarriage_is_ordinary():
    from ssda_nlp_tools.evidence import REMARRIAGE_YEARS
    s = _stats(["maria"] * 40 + ["juan perez", "pedro gomez"])
    near = network_llr(_m("maria", 1850, spouse="juan perez"),
                       _m("maria", 1852, spouse="pedro gomez"), s)[0]
    far = network_llr(_m("maria", 1810, spouse="juan perez"),
                      _m("maria", 1810 + REMARRIAGE_YEARS + 30,
                         spouse="pedro gomez"), s)[0]
    assert far > near, "widowhood then remarriage is not a contradiction"


def test_one_parent_each_is_a_mother_and_a_father_not_a_conflict():
    """The false positive the A/B audit caught.

    Two 1742 entries for "Maria de Jesus" naming the SAME husband were split
    because one recorded parent=elena and the other parent=francisco. `parent`
    has capacity two and the registers name whichever one they please.
    """
...
def test_three_distinct_parents_between_them_is_impossible():
    ...
    a = {"name": "josefa", "_year": 1850,
         "_ctx": {("parent", "ana lopez"), ("parent", "juan lopez")}}
    b = {"name": "josefa", "_year": 1851, "_ctx": {("parent", "ines vega")}}
    assert _conflicts(a, b, s), "nobody has three parents"
...
def test_conflicts_are_penalties_never_vetoes():
    """The relation names come from an LLM and can be wrong, so a conflict must
    stay recoverable by other evidence rather than returning -inf."""
    s = _stats(["maria"] * 40 + ["juan perez", "pedro gomez"])
